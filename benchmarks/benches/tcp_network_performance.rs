...
fn benchmark_p2p_throughput(c: &mut Criterion) {
    ...
    let configs = vec![
        NetworkBenchConfig { node_count: 2, message_count: 100, ..Default::default() },
        NetworkBenchConfig { node_count: 2, message_count: 500, ..Default::default() },
        NetworkBenchConfig { node_count: 2, message_count: 1000, ..Default::default() },
    ];
...
fn benchmark_broadcast_performance(c: &mut Criterion) {
    ...
    let configs = vec![
        NetworkBenchConfig { node_count: 3, message_count: 50, ..Default::default() },
        NetworkBenchConfig { node_count: 5, message_count: 50, ..Default::default() },
        NetworkBenchConfig { node_count: 7, message_count: 50, ..Default::default() },
    ];
                ...
                b.to_async(&rt).iter_custom(|iters| async move {
                    ...
                    for _ in 0..iters {
                        ...
                        for i in 0..config.message_count {
                            ...
                            black_box(broadcaster.broadcast(message, Some(broadcaster.node_id)).await.unwrap());
                        }
            ...
            |b, &concurrent_senders| {
                b.to_async(&rt).iter_custom(|iters| async move {
                    ...
                    for _ in 0..iters {
                        ...
                        let sender_networks: Vec<_> = networks[..concurrent_senders].iter().cloned().collect();
                        ...
                        let expected_messages = (config.message_count / concurrent_senders) * concurrent_senders;
