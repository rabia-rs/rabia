...
use rabia_core::batching::{BatchConfig, BatchProcessor, CommandBatcher};
        ...
        b.iter_batched(
            ...
            |commands| {
                ...
                for command in commands {
                    ...
                    let serialized = json_serializer
                        .serialize_message(black_box(&message))
                        .unwrap();
            ...
            |commands| {
                ...
                for command in commands {
                    if let Some(batch) = batcher.add_command(command).unwrap() {
                        ...
                        let serialized = binary_serializer
                            .serialize_message(black_box(&message))
                            .unwrap();
                        results.push(serialized);
                    }
