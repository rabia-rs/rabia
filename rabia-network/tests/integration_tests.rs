...
use rabia_core::{NodeId, messages::*, network::NetworkTransport};
...
async fn test_tcp_message_exchange() {
    ...
    network1.send_to(node2_id, test_message.clone()).await.unwrap();
    
    // Receive message on node2
    let (from, received_message) = tokio::time::timeout(
    ...
    network2.broadcast(broadcast_message.clone(), Some(node2_id)).await.unwrap();
    
    // Receive broadcast on node1
    let (from, received_broadcast) = tokio::time::timeout(
        Duration::from_secs(5),
        network1.receive()
    ).await.unwrap().unwrap();
...
async fn test_tcp_cluster_formation() {
        ...
        for j in 0..NUM_NODES {
            if i != j {
                ...
                if let Err(e) = networks[i].connect_to_peer(target_node, target_addr).await {
                    warn!("Failed to connect node {} to {}: {}", nodes[i], target_node, e);
                }
            }
        }
    }
    ...
    for i in 0..NUM_NODES {
        ...
        info!("Node {} connected to {} other nodes", nodes[i], connected.len());
        
        // Each node should be connected to all other nodes
        assert!(connected.len() >= NUM_NODES - 2, 
            "Node {} only connected to {} nodes, expected at least {}", 
            nodes[i], connected.len(), NUM_NODES - 2);
    }
    ...
    networks[broadcaster_idx].broadcast(
        broadcast_message.clone(), 
        Some(nodes[broadcaster_idx])
    ).await.unwrap();
    ...
    info!("Broadcast received by {}/{} nodes", received_count, NUM_NODES - 1);
    assert!(received_count >= (NUM_NODES - 1) / 2, "Too few nodes received broadcast");
...
async fn test_tcp_fault_tolerance() {
    ...
    network1.send_to(node3_id, test_message.clone()).await.unwrap();
    
    let (from, _) = tokio::time::timeout(
...
async fn test_tcp_high_volume() {
    ...
    info!("Received {}/{} messages in high volume test", received_count, MESSAGE_COUNT);
    assert!(received_count >= MESSAGE_COUNT * 8 / 10, 
        "Received only {}/{} messages", received_count, MESSAGE_COUNT);
...
async fn test_tcp_concurrent_operations() {
    ...
    // Send messages rapidly (simulating concurrent operations)
    ...
    info!("Received {}/{} rapid messages", received_count, CONCURRENT_TASKS);
    assert!(received_count >= CONCURRENT_TASKS * 8 / 10,
        "Only received {}/{} rapid messages", received_count, CONCURRENT_TASKS);
