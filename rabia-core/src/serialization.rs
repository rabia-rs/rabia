...
use crate::{Result, RabiaError, messages::ProtocolMessage, memory_pool::{get_pooled_buffer, PooledBuffer}};
...
impl Default for Serializer {
    fn default() -> Self {
        Self::Binary(BinarySerializer::default())
    }
}
...
impl SerializerFactory {
    pub fn create(config: &SerializationConfig) -> Serializer {
        if config.use_binary {
            Serializer::Binary(BinarySerializer::default())
        } else {
            Serializer::Json(JsonSerializer::default())
        }
    }
}
...
fn estimate_message_size(message: &ProtocolMessage) -> usize {
    ...
    let payload_size = match &message.message_type {
        ...
        MessageType::NewBatch(new_batch) => {
            32 + new_batch.batch.commands.len() * 64
        }
        MessageType::HeartBeat(_) => 24,
        MessageType::QuorumNotification(notif) => {
...
mod tests {
    use super::*;
    use crate::messages::*;
    use crate::{NodeId, PhaseId, StateValue, CommandBatch, Command};
    ...
    fn test_pooled_serialization() {
        ...
        let deserialized = serializer.deserialize_message(pooled_buffer.as_slice()).unwrap();
        assert_eq!(message.from, deserialized.from);
    }
}